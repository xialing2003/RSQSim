# totally based on simulator_v2.py, just include parallel computing
...
def update_step(indx, Dtau, Dtaup, taudot, velocity, q, Kjk, slip,
    ...
    for j in prange(my):
        ...
        for k in range(nx):
            if indx[j, k] == 0:
                Dttest = 0.0
                local_dt = ((omaob * np.log(q[j,k])) - Dtau[j,k]) / taudot[j,k]
                while abs(local_dt - Dttest) > 1e-5 * abs(local_dt):
                    Dttest = local_dt
                    local_dt = ((omaob * np.log(q[j,k]+Dttest)) - Dtau[j,k]) / taudot[j,k]
...
if __name__ == "__main__":
    ...
    folder = '../results/test_RSQSim/test_6.6/'
    ...
    while istep < istep_record:
        ...
        dtnext, jj, kk, Dtau, taudot, indx, velocity, q, slip = update_step(indx, Dtau, Dtaup, taudot, velocity, q, Kjk, slip,
                                                                            my, nx, overshoot, Dtaupmin, aob, Veq_n)
