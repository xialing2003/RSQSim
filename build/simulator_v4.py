...
# simulator_3.py: don't consider any effects caused by nucleation
# simulator_v3.py: the nucleation will cause stress increase on other elements, and stress decrease on its own,
#                   which is considered through changing the stressing rate function
# simulator_v4.py: the nucleation just have stress decrease on itself and the stressing rate is not influenced at all.
...
if __name__ == "__main__":
    ...
    folder = '../results/RSQSim_stage4/test_v4_Dc2um/'
    ...
    # record the nucleation velocity distribution
    bins = np.linspace(1, 1000, 1001)   # 100 bins
    ...
    #     selected = values[coords[:,0], coords[:,1]]

    #     h, _ = np.histogram(selected, bins=bins)
    #     hist += h
    ...
    while istep < istep_record:
        ...
        indx[jj, kk] = idx_to_change
        
        if dtnext < 0:
            print('Wrong!')

        nuc_MR_np[istep] = velocity[state_j[1, :state_n[1]], state_k[1, :state_n[1]]].sum()
        nuc_velocity = velocity[state_j[1, :state_n[1]], state_k[1, :state_n[1]]]
        h, _ = np.histogram(nuc_velocity, bins=bins)
        hist += h

        active = indx[js, ks] == 1
        if np.any(active):
            nuc_indx.extend(np.where(active)[0])
            nuc_stress.extend(Dtau[js[active], ks[active]])
            nuc_vel.extend(velocity[js[active], ks[active]])
    ...
    plt.xlabel(r"Normalized Nucleation Velocity ($V/V_{pl}$)")
    plt.ylabel("Count")
    plt.savefig(folder + 'hist_nuc_vel.png')
    np.save(folder + 'nuc_velocity_hist.npy', hist)

    nuc_indx = np.asarray(nuc_indx, dtype=np.int8)
    nuc_stress = np.asarray(nuc_stress)
    nuc_vel = np.asarray(nuc_vel)
    np.savez(
        folder + 'nucleation_data.npz',
        element_index=nuc_indx,
        stress=nuc_stress,
        velocity=nuc_vel
    )