...
@njit
def update_stress_rate1(K_sym, velocity, nuc_j, nuc_k, number_nuc, V_c):

    taudot1 = np.zeros((my, nx), dtype = np.float64)
    for i in range(number_nuc):
        jj = nuc_j[i]
        kk = nuc_k[i]
        jstart = my - jj - 1
        kstart = nx - kk - 1
...
def update_step(indx, Dtau, Dtaup, taudot, velocity, q, Kjk, slip, slip_nuc,
    ...
    return dtnext, jj, kk, Dtau, indx, velocity, q, slip, slip_nuc, MR_nuc
...
@njit
def run_simulation(istep_record, my, nx, aob, Veq_n, size_rec, Dtau, Dtaupmin, \
                   taudot0, indx, velocity, q, Kjk, val):
    tim = 0.0
    nucleation_number = 0
    slip_number = 0
    istep = 0
    ...
    # a list for nucleation elements
    number_nuc = 0
    nuc_j = np.zeros(my*nx, dtype=np.int32)
    nuc_k = np.zeros(my*nx, dtype=np.int32)
    nuc_label = np.zeros((my, nx), dtype=np.int32)

    while istep < istep_record:

        dtnext, jj, kk, Dtau, indx, velocity, q, slip, slip_nuc, MR_nuc \
        ...
        if indx[jj, kk] == 2:
            ...
            number_nuc -= 1
            nuc_j[nuc_label[jj, kk]] = nuc_j[number_nuc]
            nuc_k[nuc_label[jj, kk]] = nuc_k[number_nuc]
            nuc_label[nuc_j[number_nuc], nuc_k[number_nuc]] = nuc_label[jj, kk]
        elif indx[jj, kk] == 1:
            nucleation_number += 1

            nuc_j[number_nuc] = jj
            nuc_k[number_nuc] = kk
            nuc_label[jj, kk] = number_nuc
            number_nuc += 1
        else:
            slip_number -= 1

        ico = ico_label[indx[jj, kk]]
        taudot2 = update_stress_rate2(taudot2, ico * Veq_n, jj, kk, K_sym)
        taudot1 = update_stress_rate1(K_sym, velocity, nuc_j, nuc_k, number_nuc, val * Veq_n)
        taudot = taudot0 + taudot1 + taudot2
...
if __name__ == "__main__":
    ...
    iplotslip, select_nuc, nuc_count = run_simulation(istep_record, my, nx, aob, \
                    Veq_n, size_rec, Dtau, Dtaupmin, taudot0, indx, velocity, q, Kjk, val)
    end = time.time()
